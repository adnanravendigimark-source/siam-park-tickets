...
const CONTENT_SECTIONS = [
  ...
  { id: "sec-toprides", label: "Top Rides" },
...
export default function HomepageForm({ initial, tours }: { initial: HomepageContent; tours: Tour[] }) {
  ...
  function updateTopRides(patch: Partial<HomepageContent["sections"]["topRides"]>) {
    setContent((c) => ({ ...c, sections: { ...c.sections, topRides: { ...c.sections.topRides, ...patch } } }));
    setSaved(false);
  }
    ...
    <form onSubmit={handleSubmit} className="space-y-5 pb-24">
      ...
      {activeTab === "content" && (
        <div className="space-y-5">
          ...
          <SectionCard
            id="sec-toprides"
            title="Top Rides section"
            description="Images live on the Images tab."
            open={!!openSections["sec-toprides"]}
            onToggle={() => toggleSection("sec-toprides")}
          >
            <div className="grid gap-5 sm:grid-cols-2">
              <Field label="Eyebrow label">
                <input value={content.sections.topRides.eyebrow} onChange={(e) => updateTopRides({ eyebrow: e.target.value })} className={inputClass} />
              </Field>
              <Field label="Heading (H2)">
                <input value={content.sections.topRides.heading} onChange={(e) => updateTopRides({ heading: e.target.value })} className={inputClass} />
              </Field>
            </div>
            <Field label="Body text">
              <RichTextEditor value={content.sections.topRides.body} onChange={(html) => updateTopRides({ body: html })} />
            </Field>
            <Field label="Bullet points">
              <RepeatableList<string>
                items={content.sections.topRides.bullets}
                onChange={(bullets) => updateTopRides({ bullets })}
                newItem={() => ""}
                addLabel="+ Add bullet"
                renderItem={(item, upd) => (
                  <input value={item} onChange={(e) => upd(e.target.value)} className={inputClass} />
            ...
            <div className="grid gap-5 sm:grid-cols-2">
              <Field label="Button text">
                <input value={content.sections.topRides.ctaButtonText} onChange={(e) => updateTopRides({ ctaButtonText: e.target.value })} className={inputClass} />
              </Field>
              <Field label="Button link">
                <input value={content.sections.topRides.ctaHref} onChange={(e) => updateTopRides({ ctaHref: e.target.value })} className={inputClass} />
              </Field>
            </div>
          </SectionCard>
      ...
      {activeTab === "images" && (
        <div className="space-y-5">
          ...
          <SectionCard title="Top Rides photos">
            <RepeatableList<GalleryImage>
              items={content.sections.topRides.images}
              onChange={(images) => updateTopRides({ images })}
        ...
        <div className="space-y-5">
          <SectionCard title="Focus keyword" description="The main phrase you want the homepage to rank for. Purely a writing aid — nothing here is sent to Google.">
            <Field label="Focus keyword">
              <input value={content.focusKeyword} onChange={(e) => update("focusKeyword", e.target.value)} className={inputClass} placeholder="e.g. Siam Park Tickets" />
            </Field>
            {focusChecklist && (
